        ...
        if( $('#product-form').length ) {
            ...
            else
            {
               ...
               $('#add-product').hide(); 
                ...
                $.ajax({
                    url: 'http://localhost:'+port+'/costs/frontend/WEB/index.php?r=invoice/productinfo',
                    type: 'POST',
                    data: {
                    ...
                    success: function (reply) {
                        ...
                        if( data.type == 'success' ) {
                            ...
                            if( data.imgUrl !== '')
                                html += '<img src="http://localhost:'+port+'/costs/backend/web/'+ data.imgUrl +'" alt="" style="max-width: 150px; max-height: 150px;margin-top:10px" /></p>'
                            else
            ...
            $('#add-product').on('click', function(event)  {
                ...
                if( quantity <= 0 || isNaN(quantity) || quantity == '') {
                    alertM('danger', 'You need to have quantity equal to 1 or greater.');
...
if( $('#product-form').length ) {
    ...
    function insertNewProduct2Json(object) {
       ...
       if( !$.isEmptyObject(products) ) {
        ...
        alertM('success', 'Product was added.');
        
    }

    function alertM(type, message) {
        if( type == 'success' ) {
